...
export default function PreMentorLoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/auth/login", {
                ...
                body: JSON.stringify({ ...formData, role: "prementor" }),
            });
            ...
            if (res.ok) {
                ...
                localStorage.setItem("userRole", "prementor");
                
                // Check if user has already completed onboarding
                console.log("Checking onboarding for user:", data.user._id || data.user.id);
                const userId = data.user._id || data.user.id;
                if (!userId) {
                    console.error("No user ID found in login response:", data.user);
                    alert("Login error: Missing user ID");
                    return;
                }
                ...
                // Redirect to dashboard if onboarding completed, otherwise to onboarding
                if (onboardingData.hasOnboarding) {
                    console.log("Redirecting to dashboard");
                    window.location.href = "/dashboard/prementor";
                } else {
                    console.log("Redirecting to onboarding");
                    window.location.href = "/onboarding/prementor";
                }
            } else {
                ...
                <div className="w-full max-w-md bg-white rounded-[30px] p-8 md:p-10 shadow-[0_20px_60px_rgba(14,165,233,0.1)]">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div className="space-y-4">
                            ...
                            <button 
                                type="button" 
                                onClick={() => import("next-auth/react").then(mod => mod.signIn('google', { callbackUrl: `/auth/redirect?role=prementor` }))} 
                        ...
                        <div className="text-center pt-4">
                            ...
                            <Link href="/signup/prementor" className="text-[#0EA5E9] font-bold text-sm hover:underline">
                                Register now
                            </Link>
                        </div>
                ...
                <div className="flex flex-col items-center flex-1">
                    ...
                    <div className="relative w-[360px] md:w-[500px] lg:w-[650px] -mb-10 lg:-mb-20 flex justify-center">
                        <Image
                            ...
                            width={90000}
                            height={90000}
                            className="w-2/3 h-auto drop-shadow-2xl z-10 relative"
                            priority
                        />
