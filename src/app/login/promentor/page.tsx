...
export default function ProMentorLoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/auth/login", {
                ...
                body: JSON.stringify({ ...formData, role: "promentor" }),
            });

            const data = await res.json();

            console.log('[ProMentor Login] Response:', data);
            console.log('[ProMentor Login] User role:', data.user?.role);

            if (res.ok) {
                // Store minimal user info and role
                localStorage.setItem("user", JSON.stringify(data.user));
                localStorage.setItem("userRole", "promentor");

                const userId = data.user?._id || data.user?.id;
                const userRole = data.user?.role;
                
                console.log('[ProMentor Login] User role:', userRole, 'ID:', userId);

                // Check if user is approved promentor
                if (userRole === 'promentor') {
                    // Check if onboarding is complete
                    const onboardingRes = await fetch(`/api/auth/onboarding/check?userId=${userId}`);
                    const onboardingData = await onboardingRes.json();
                    
                    console.log('[ProMentor Login] Onboarding check:', onboardingData);
                    
                    if (onboardingData.hasOnboarding) {
                        console.log('[ProMentor Login] Onboarding complete, redirecting to dashboard');
                        router.push('/dashboard/promentor');
                    } else {
                        console.log('[ProMentor Login] Onboarding needed, redirecting to onboarding');
                        router.push('/onboarding/promentor');
                    }
                } else if (userRole === 'prementor' || userRole === 'mentee') {
                    console.log('[ProMentor Login] User needs to complete promentor onboarding');
                    router.push('/onboarding/promentor');
                } else {
                    console.log('[ProMentor Login] User not approved yet, showing error');
                    setError(`Your application is pending admin approval. Current role: ${userRole || 'unknown'}`);
                }
            } else {
                if (data.message?.includes("registered as")) {
                    alert("Your application is pending admin approval. Please wait for verification before logging in.");
                } else {
                ...
                <div className="w-full max-w-md bg-white rounded-[30px] p-8 md:p-10 shadow-[0_20px_60px_rgba(14,165,233,0.1)]">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div className="text-right">
                            <Link href="/forgot-password" className="text-[#0EA5E9] text-sm font-semibold hover:underline">
                                Forgot password?
                            </Link>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <button 
                                type="button" 
                                onClick={() => import("next-auth/react").then(mod => mod.signIn('google', { callbackUrl: `/auth/redirect?role=promentor` }))} 
                        ...
                        <div className="text-center pt-4">
                            ...
                            <Link href="/signup/promentor" className="text-[#0EA5E9] font-bold text-sm hover:underline">
                                Register now
                            </Link>
                        </div>
                ...
                <div className="flex flex-col items-center flex-1">
                    ...
                    <div className="relative w-[360px] md:w-[500px] lg:w-[650px] -mb-10 lg:-mb-20 flex justify-center">
                        <Image
                            ...
                            alt="ProMentor Character"
                            width={90000}
                            height={90000}
                            className="w-5/6 h-auto drop-shadow-2xl z-10 relative"
                            priority
                        />
