    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/auth/login", {
                ...
                body: JSON.stringify({ ...formData, role: "mentee" }),
            });
            ...
            if (res.ok) {
                ...
                // Route based on onboarding: if currentStatus not set, go to onboarding
                const onboarding = data.user?.onboarding;
                const hasOnboarded = onboarding?.currentStatus && onboarding.currentStatus.length > 0;
                if (hasOnboarded) {
                ...
                <div className="w-full max-w-md bg-white rounded-[30px] p-8 md:p-10 shadow-[0_20px_60px_rgba(14,165,233,0.1)]">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div className="text-center pt-4">
                            ...
                            <Link href="/signup/mentee" className="text-[#0EA5E9] font-bold text-sm hover:underline">
                                Register now
                            </Link>
                        </div>
